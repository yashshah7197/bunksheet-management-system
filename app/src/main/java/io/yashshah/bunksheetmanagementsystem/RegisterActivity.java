...
public class RegisterActivity extends AppCompatActivity {
  ...
  private void setupViews() {
    mFirstNameInputLayout = (TextInputLayout) findViewById(R.id.textInputLayout_register_firstName);
    mFirstNameInputLayout.getEditText()
        .setInputType(
            InputType.TYPE_TEXT_VARIATION_PERSON_NAME | InputType.TYPE_TEXT_FLAG_CAP_WORDS);

    mLastNameInputLayout = (TextInputLayout) findViewById(R.id.textInputLayout_register_lastName);
    mLastNameInputLayout.getEditText()
        .setInputType(
            InputType.TYPE_TEXT_VARIATION_PERSON_NAME | InputType.TYPE_TEXT_FLAG_CAP_WORDS);

    mEmailInputLayout = (TextInputLayout) findViewById(R.id.textInputLayout_register_email);

    mPasswordInputLayout = (TextInputLayout) findViewById(R.id.textInputLayout_register_password);
    ...
    confirmPasswordEditText.addTextChangedListener(new TextWatcher() {
      ...
      @Override public void onTextChanged(CharSequence sequence, int start, int before, int count) {
        if (passwordsMatch(passwordEditText.getText(), sequence)) {
          ...
          mConfirmPasswordInputLayout.setError(getString(R.string.passwords_not_matching));
        }
      }
      ...
      mValueEventListener = new ValueEventListener() {
        @Override public void onDataChange(DataSnapshot dataSnapshot) {
          if (!dataSnapshot.hasChild(mCurrentUser.getUid())) {
            ...
            mUsersDatabaseReference.child(mCurrentUser.getUid())
                .setValue(newUser)
  ...
  private boolean isValidPassword(CharSequence password) {
    return (!TextUtils.isEmpty(password) && password.length() >= 6 && (!password.toString()
        .contains(" ")));
  }
  ...
  private boolean userDetailsValid() {
    ...
    return (isValidFirstName(mFirstName)
        && isValidLastName(mLastName)
        && isValidEmail(mEmail)
  ...
  private void updateUserDetails() {
    UserProfileChangeRequest changeRequest =
        new UserProfileChangeRequest.Builder().setDisplayName(mFirstName + " " + mLastName).build();

    mCurrentUser.updateProfile(changeRequest).addOnCompleteListener(new OnCompleteListener<Void>() {
      @Override public void onComplete(@NonNull Task<Void> task) {
  ...
  private void setSharedPrefsPrivilegeLevel() {
    DatabaseReference privilegeDatabaseReference = mFirebaseDatabase.getReference()
        .child("Users")
