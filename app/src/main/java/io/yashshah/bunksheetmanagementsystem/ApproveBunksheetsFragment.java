...
public class ApproveBunksheetsFragment extends Fragment {
  ...
  private boolean isProfileFilled() {
    if (mUser.getName().equals("") || mUser.getPhoneNumber().equals("") || mUser.getYear()
        .equals("") || mUser.getDivision().equals("") || mUser.getClassTeacher().equals("") || mUser
        .getTeacherGuardian()
        .equals("") || mUser.getRollNumber().equals("")) {
      return false;
    }
    return true;
  }
      ...
      mItemTouchHelperCallback = new ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP,
        ...
        @Override public void onChildDraw(Canvas c, RecyclerView recyclerView,
          ...
          if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            ...
            if (dX > 0) {
              mPaint.setColor(ContextCompat.getColor(getActivity(), R.color.colorGreen));
  ...
  private void attachRecyclerViewAdapter() {
    Query query =
        mDatabaseReference.orderByChild("approvalLevel").equalTo(mUser.getPrivilegeLevel() - 1);

    if (mFirebaseRecyclerAdapter == null) {
      mFirebaseRecyclerAdapter =
          new FirebaseRecyclerAdapter<Bunksheet, BunksheetViewHolder>(Bunksheet.class,
              R.layout.bunksheet_list_item, BunksheetViewHolder.class, query) {
            @Override
    ...
    ValueEventListener valueEventListener = new ValueEventListener() {
      @Override public void onDataChange(DataSnapshot dataSnapshot) {
        ...
        if (bunksheet.getApprovalLevel() == mUser.getPrivilegeLevel()) {
          Toast.makeText(getActivity(), getString(R.string.already_approved), Toast.LENGTH_LONG)
              .show();
        } else {
          if (approved) {
            ...
            if (bunksheet.getApprovedBy().equals(getString(R.string.na))) {
              ...
              approvedBy.append(bunksheet.getApprovedBy()).append(", ").append(mUser.getName());
            }
            ...
            databaseReference.updateChildren(childMap)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                  @Override public void onComplete(@NonNull Task<Void> task) {
                    if (isAdded()) {
                      Toast.makeText(getActivity(), getString(R.string.approved_successfully),
            ...
            Map<String, Object> childMap = new HashMap<>();
            childMap.put("approvalLevel", -1);
            childMap.put("approvedBy", "Declined");
            databaseReference.updateChildren(childMap)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                  @Override public void onComplete(@NonNull Task<Void> task) {
                    if (isAdded()) {
                      Toast.makeText(getActivity(), getString(R.string.bunksheet_decline_success),
                          Toast.LENGTH_LONG).show();
                    }
                  }
                });
