...
public class BunksheetsFragment extends Fragment {
    ...
    if (mItemTouchHelperCallback == null) {
      mItemTouchHelperCallback = new ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP,
        ...
        @Override public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
          ...
          Bunksheet bunksheet = (Bunksheet) mFirebaseRecyclerAdapter.getItem(position);
          if (bunksheet.getApprovalLevel() == -1) {
            if (isAdded()) {
              Toast.makeText(getActivity(), getString(R.string.bunksheet_delete_error),
  ...
  private void deleteBunksheetFromDatabase(String bunksheetKey) {
    ...
    databaseReference.removeValue(new DatabaseReference.CompletionListener() {
      ...
      public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
        if (isAdded()) {
          Toast.makeText(getActivity(), getString(R.string.bunksheet_delete_success),
              Toast.LENGTH_LONG).show();
        }
      }
    });
  ...
  private void showConfirmDeleteAlertDialog(final String bunksheetKey, final int position) {
    ...
    builder.setPositiveButton(getString(R.string.yes_confirm),
        new DialogInterface.OnClickListener() {
